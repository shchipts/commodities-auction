  ...
  (testing "with outcomes for every iteration"
    (; Act
     let [auction1 (run {:c1 5 :c2 7 :c3 4}
      ...
      ; Assert
      (is (real= (get-in auction1 [:suppliers :c1])
                 [0 0.05 0.35	0.4	0.45]))
      (is (real= (get-in auction1 [:suppliers :c2])
                 [0	0	0.3 0.3 0.35]))
      (is (real= (get-in auction1 [:suppliers :c3])
                 [0 0.05 0.35	0.4	0.45]))
      ...
      (is (real= (get-in auction2 [:suppliers :c1])
                 [0 0.058823529	0.117647059]))
      (is (real= (get-in auction2 [:suppliers :c2])
                 [0 0 0.058823529]))
      (is (real= (get-in auction2 [:suppliers :c3])
                 [0 0 0]))
      (is (real= (get-in auction2 [:suppliers :c4])
                 [0 0 0]))
  ...
  (testing "with surpluses as outcomes"
    (; Act
     let [auction1 (run {:c1 5 :c2 7 :c3 4}
      ...
      (is (real= (get-in auction3 [:suppliers :c1])
                 0.65))
      (is (real= (get-in auction3 [:suppliers :c2])
                 0.65))
      (is (real= (get-in auction3 [:suppliers :c3])
                 0.65))
      ...
      (is (real= (get-in auction4 [:suppliers :c1])
                 0.2))
      (is (real= (get-in auction4 [:suppliers :c2])
                 0.1))
      (is (real= (get-in auction4 [:suppliers :c3])
                 0.1))
      ...
      (is (real= (get-in auction5 [:suppliers :c1])
                 0.117647059))
      (is (real= (get-in auction5 [:suppliers :c2])
                 0.058823529))
      (is (real= (get-in auction5 [:suppliers :c3])
                 0))
      (is (real= (get-in auction5 [:suppliers :c4])
                 0))
  ...
  (testing "with non-competitive imports"
    (; Act
     let [auction (run {:c1 4 :c2 2 :c3 5 :c4 3}
      ...
      ; Assert
      (is (real= (get-in auction [:suppliers :c1])
                 0.176470588))
      (is (real= (get-in auction [:suppliers :c2])
                 0.235294118))
      (is (real= (get-in auction [:suppliers :c3])
                 0))
      (is (real= (get-in auction [:suppliers :c4])
                 0))
